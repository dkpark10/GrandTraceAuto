...
parser.add_argument(
    ...
    default=False)
parser.add_argument(
    '--dktest', action='store',dest='dktest',
    default = False)
...
professor = Model(inputs=c_model.teacher_model.input,
...
print()
srgan_init = SRGAN(d_model, g_model, professor, 64, 64, 32, 32, 12)
srgan_model = srgan_init.srgan_model

###### Model Complile ###############################################
print('\nCheck interm1 Layers')
for layer in professor.layers:  # VGG trainable = False

    layer.trainable = False
    print(layer, layer.trainable)

professor.compile(loss='mse', optimizer=optimizer, metrics=['accuracy'])
...
if parsed.gt:
    ...
    batched_hr_d = batch_generator(x_train, batch_size=32)
    batched_lr_d = []
    ...
    a_batchg_lr_G = []
    for i in range(10):
        a_batchg_lr_G.append(fat_lr_bg(x_low_train[i], batch_size=32))
    a_batchg_hr_G = []
    for i in range(10):
    ...
    for GTA_e in range(10000):
        ...
        for i in range(1):
            j = np.random.randint(10)
            hr_feature = professor.predict(next(a_batchg_hr_G[j]))
            tensorboard = TensorBoard(log_dir="logs/{}".format(time()))
            srgan_model.fit(next(a_batchg_lr_G[j]), [hr_vailidity, hr_feature], callbacks = [tensorboard])

        print('GLOBAL EPOCH NO. ', GTA_e, '<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<' * 2)
        if (GTA_e) % 2000 is 0:

            a_image = g_model.predict_generator(a_batchg_lr_G[8], steps=1)[1]
            #a_image = g_model.predict_generator(a_batchg_lr_G[8], steps=1)
            #a_image = a_image[1]
